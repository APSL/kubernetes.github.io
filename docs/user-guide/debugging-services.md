...
For example, Alpine Linux prior to the 3.4 version would not support the `search`
keyword in `resolv.conf`. If you want to use Alpine as base image you will need
to get an specially built for Kubernetes or update to at least Alpine 3.4.
...
* Does the service has a `selector` and is it spelled correctly?
